...
function getVehicle(code, onComplete = null) {
  ...
  xhr.open("GET", restapi + "/api/vehicles?code=" + encodeURIComponent(code), true);
...
function getVehicleById(id, onComplete = null) {
  ...
  xhr.open("GET", restapi + "/api/vehicles/" + encodeURIComponent(id), true);
...
function restoreCurrentAnswer() {
  ...
  if (current.oilPhoto) {
     ...
     */
    showQuestionPhotoPreview("oilPhotoPreview", current.oilPhoto);
  }
  ...
  if (current.answer === "Report Faults") {
    ...
    if (current.error?.photo) {
      ...
      showQuestionPhotoPreview("faultPhotoPreview", current.error.photo);
    }
  }
}
...
function showSummaryModal(inspectionResult) {
  ...
  if (answersContainer) {
    ...
    inspectionResult.answers.forEach((item, index) => {
      ...
      answerText.innerHTML = `<strong>Vastaus:</strong> ${escapeHTML(formatSummaryAnswer(item.answer))}`;
      ...
      if (item.answer?.oilPhoto) {
        appendSummaryPhoto(card, item.answer.oilPhoto, "Öljymittatikun kuva");
      }
      ...
      if (item.answer?.error) {
        ...
        if (fault.priority) {
          ...
          priority.textContent = `Prioriteetti: ${getPriorityLabel(fault.priority)}`;
         ...
         */
        if (fault.photo) {
          appendSummaryPhoto(card, fault.photo, "Kuva viasta");
        }
      }
...
function renderFuelGauge(container) {
  ...
  if (fuelFaultButton) {
    fuelFaultButton.addEventListener("click", () => {
      selectFault();
    });
  }
